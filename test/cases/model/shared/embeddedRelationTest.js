...
describeWith = function(store) {
  ...
  describe("Tower.ModelRelationHasMany (Tower.Store." + store.name + ")", function() {
    beforeEach(function(done) {
      App.Post.store(store);
      App.Comment.store(store);
      App.Post.destroy(function() {
        App.Comment.destroy(function() {
          App.Post.create({rating: 5, id: 10, title: "A Post"}, function(error, record) {
            post = record;
            done();
          });
        });
    ...
    if (store !== Tower.StoreMemory) {
      describe('embedded', function() {
        describe('.create', function() {
          test('existingPost.embeddedComments().create()', function(done) {
            post.embeddedComments().create(function(error, comment) {
              assert.equal(comment.constructor.name, "Comment");
              App.Post.store().findOne({id: 10}, {raw: true}, function(error, doc) {
                assert.equal(doc.embeddedComments.length, 1);

                App.Comment.count(function(error, count) {
