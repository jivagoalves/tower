describe('Tower.ModelValidation', function() {
  var criteria, record, scope, user, validator, field;
  ...
  it('should validate from attribute definition', function() {
    var page = App.Page.build({title: "A App.Page"});
  ...
  describe('presence', function() {
    test('field "name", required: true', function() {
      var field = App.Validatable.fields().string_requiredTrue;
      var validators = field.validators();
      assert.equal(validators.length, 1);
      assert.equal(validators[0].name, "required");
    });

    test('field "name"; @validates "name", presence: true', function() {
      var field = App.Validatable.fields().string_presenceTrue;
      var validators = field.validators();
      assert.equal(validators.length, 1);
  ...
  describe('format', function() {
    ...
    test('field "name", format: value: /^\d+$/', function() {
  ...
  describe('enumerable', function() {
    ...
    test('field "name", notIn: ["male", "female"]', function() {
  ...
  describe('multiple fields at once', function() {
    test('field "name", "name2", presence: true, format: /^[a-z]+/, on: "create"', function() {
      var a = App.Validatable.validators("onCreate_all1");
      var b = App.Validatable.validators("onCreate_all2");
      var all = _.flatten(a, b);
      assert.equal(a.length, 2);
      assert.equal(a.length, b.length);
      
      _.each(all, function(validator) {
    ...
    test('field "name", "name2", presence: true, format: value: /^[a-z]+/, on: "create"', function() {
      ...
      for (_j = 0, _len1 = presenceValidators.length; _j < _len1; _j++) {
        ...
        _results.push(assert.equal(presenceValidator.options.on, undefined));
      }
    });
    ...
    test('field "name", "name2", presence: {if: "method"}, format: unless: "method", value: /^[a-z]+/, on: "create"', function() {
      ...
      for (_j = 0, _len1 = presenceValidators.length; _j < _len1; _j++) {
        ...
        _results.push(assert.equal(presenceValidator.validateEach(record, record.get('errors')), false));
      }
    });

    test('field "name", "name2", presence: true, format: /^[a-z]+/, if: "method"', function() {
      ...
      for (_i = 0, _len = all.length; _i < _len; _i++) {
        ...
        _results.push(assert.equal(validator.validateEach(record, record.get('errors')), false));
      }
    });
  });
});
