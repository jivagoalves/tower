...
describe('Tower.ControllerScopes', function() {
  var C;
  ...
  describe('@scope', function() {
    ...
    test('"highlyRated"', function() {
      ...
      assert.deepEqual(C.metadata().scopes['highlyRated'].conditions(), {
        type: 'Post',
        rating: {
          '>=': 8
        }
  ...
  describe('instance', function() {
    ...
    test('"writtenBy(currentUser)"', function() {
      C.scope('writtenBy', function() {
        return App.Post.where({userId: this.get('currentUser.id')});
      });
      assert.deepEqual(_.keys(C.metadata().scopes), ['writtenBy']);
      assert.isFunction(C.metadata().scopes['writtenBy']);
      assert.deepEqual(C.instance().getCursor('writtenBy').conditions(), {
        type: 'Post',
        userId: 5
      });
    });
  ...
  describe('resolveAgainstCursors', function() {
    describe('all', function() {
      ...
      test('any', function() {
        var post = App.Post.build({id: 5, rating: 8});
